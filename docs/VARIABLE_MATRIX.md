...
Contracts foundation (R-09 slice 1, 2026-07-24): a data-driven directed-goal
(contract) system with a persisted lifecycle that observes live authoritative
state, plus a world-save schema bump to 0.6 — see the *Contracts (directed
goals)* row.
...
| Contracts (directed goals, R-09) | `data/contracts.json` (validator-checked: unique contract `id` + objective `oid`, narrow objective/reward vocab, referenced item ids exist in `items.json`, positive counts, exactly one objective + one reward each) | `scripts/contracts/contract_model.gd` (`ContractModel`: lifecycle `available → active → completed → claimed`; observes LIVE authoritative state — `stockpile_at_least` reads `town_hall.stockpile` — with completion latching on first reach; transactional `claim` grants only through `player.inventory.add_many`, with a `can_accept` guard so it is a safe, retryable no-op and never double-pays). `game_root` owns the model, wires `contracts.evaluate()` to `town_hall.stockpile_changed` and re-evaluates on load, and exposes `serialize_contracts`/`apply_contracts` + `accept_contract`/`claim_contract`. Lifecycle is world-owned: persisted in the world save under `contracts` (only records past `available`); `save_manager.SAVE_VERSION` bumped 0.5 → 0.6 (accepts 0.6/0.5/0.4, a missing key migrates to empty). |
