...
|---|---|---|
| Equipped character art (PR-03A resolution fixed; PR-03B alignment open) | Body-specific gear/swing overlays now *resolve* against `effective_body_id()` (resolved body, else the intended body), so authored gear stays visible even when the body texture is momentarily unresolved during a character/load/world-transition/forge refresh; `refresh_presentation()` re-resolves at the equip/forge boundaries. Equipment state/effects unchanged; normal-case resolution byte-identical. *Alignment* is still off on the shorter bodies (crude helmet floats ~6px above the head on goblin/dwarf; non-human torsos sit low) — PR-03B. | Contract in `docs/CHARACTER_RENDERING_CONTRACT.md`; smoke `pr03_gear_overlay_resolves_all_bodies` + `pr03_gear_survives_body_texture_miss`; alignment tracked in `docs/PRESENTATION_RECOVERY_MATRIX.md` PR-03B. |
...
| Runtime verification (2026-07-20, post PR-03) | Documentation validators, wiki-link check (5,300+ local links), HUD source/runtime hash verification, asset audit, and the focused inventory smoke (5/5) passed. The full waited-GUI smoke is **337/337**. The arc opened at 332/334 (`fq17_hud_edit_direct_manipulation` reset + `fq09_inventory_board_drag_and_sort` drag payload), both repaired in PR-00; PR-01 added the body-variant alias-contract checks; PR-02 added `pr02_character_render_contract`; PR-03 added `pr03_gear_overlay_resolves_all_bodies` + `pr03_gear_survives_body_texture_miss`. The real-time `fq09u1_live_clip_switch` adaptive-music check occasionally cold-flakes and passes on rerun. | Continue the presentation recovery arc from PR-04 (directional action animation, code half); retain fresh HUD/inventory captures as visual regression proof. |
