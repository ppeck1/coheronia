...
| Runtime verification (2026-07-17) | Documentation validators, wiki-link check (5,330 local links), HUD source/runtime hash verification, asset audit, and the focused inventory smoke (5/5) passed. The full smoke run reported 332/334. | Repair `fq09u1_live_clip_switch` and `fq09_inventory_board_drag_and_sort`, then rerun an isolated full smoke and retain the fresh HUD/inventory captures as visual regression proof. |
