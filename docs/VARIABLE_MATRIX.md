...
| Runtime verification (2026-07-20, post PR-03B) | Documentation validators, wiki-link check (5,300+ local links), HUD source/runtime hash verification, asset audit, the gear-alignment verifier (`scripts/art/verify_gear_alignment.py`), and the focused inventory smoke (5/5) passed. The full waited-GUI smoke is **338/338**. The arc opened at 332/334 (both red checks repaired in PR-00); PR-01 added alias-contract checks; PR-02 added `pr02_character_render_contract`; PR-03A added `pr03_gear_overlay_resolves_all_bodies` + `pr03_gear_survives_body_texture_miss`; PR-03B added `pr03b_gear_overlay_offset_applied`. The real-time `fq09u1_live_clip_switch` adaptive-music check occasionally cold-flakes and passes on rerun. | Continue the presentation recovery arc from PR-04 (directional action animation, code half); retain fresh HUD/inventory captures as visual regression proof. |
