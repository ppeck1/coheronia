...
| Primary HUD dock kit (2026-07-16 stabilization) | 19 native-size RGBA assets plus `hud_dock_layout.json` in `art/source_templates/hud_dock/`, promoted one-for-one to `art/generated/ui_painted/` by `scripts/art/sync_hud_kit.py` | `hud._load_hud_kit_layout`/`_hud_kit_available` select the native kit first; JSON supplies every dock/vessel/fill/label/slot/button rectangle; runtime children own health, attunement, icons, counts, hotkeys, selection, labels, and FX. The FQ-21 sliced band and FQ-19 modular dock remain fallbacks. |
