...
| DEFERRED: underground lighting (2026-07-31) | **Known limitation, intentionally left as-is.** Underground rock stays lit when viewed from the surface because ambient darkness is a **global `CanvasModulate`** driven by the *player's* depth (`game_root.ambient_darkness_factor()`/`ambient_target_color()`, column/shaft-aware via `world.sky_line`), not per-cell — so on the surface the whole canvas incl. visible underground is daylight-tinted. **Goal:** underground dark from any viewpoint, lit only where light reaches (sun surface, open shafts, torches). **Tried + reverted (this date):** a `DirectionalLight2D` sun over a dark base — it lit terrain fine with shadows OFF (surface ~171/under ~146) but with shadows ON every full-tile `OccluderPolygon2D` **self-shadowed its own tile** → all terrain dark (~27); `cull_mode` didn't help. Shipped instead: celestial `PointLight2D.shadow_enabled` (stops additive bleed). **Paths forward:** (1) a **surface-contour `LightOccluder2D`** for the directional sun on a separate light mask (live-updated on mining) — recommended; (2) a **per-column depth shader** reusing `sky_line`/`CAVE_FADE_CELLS`. Full write-up: `docs/wiki/known_issues.md` → "Deferred Rework — Underground Lighting". | `scripts/main/game_root.gd`, `scripts/world/world.gd` (tileset occluders), `docs/wiki/known_issues.md` |
