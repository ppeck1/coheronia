...
| Runtime verification (2026-07-21, post PR-08) | Documentation validators, wiki-link check (5,300+ local links), HUD source/runtime hash verification, asset audit, the gear-alignment verifier (`scripts/art/verify_gear_alignment.py`), and the focused inventory smoke (5/5) passed. The full waited-GUI smoke is **345/345**. The arc opened at 332/334 (repaired in PR-00); PR-01..PR-04 added the alias, render-contract, gear resolution/alignment, and action-swing checks; PR-05 added `pr05_preview_matches_world_render`; PR-06 added `pr06_character_panel_runtime_render`; PR-07 added `pr07_backdrop_contour_skirt_follows_surface`; PR-08 added `pr08_skill_panel_viewport_relative`. Two runtime notes: the real-time `fq09u1_live_clip_switch` music check occasionally cold-flakes and passes on rerun, and `fq19_map_events_coexist` is sensitive to a contaminated persisted `shell.json` (a corrupt HUD layout `reset_hud_layout()` cannot fully neutralise) -- it passes from a clean shell profile. | The presentation recovery arc's code lane (PR-00..PR-08) is complete; remaining PR-09 (skill expansion) is deferred and PR-10 (HUD chrome) is an art lane. |
