...
| PR-03B | code + art | Gear overlay alignment | **OPEN.** Body-specific crude overlays sit wrong on the shorter bodies: the crude helmet floats ~6px above the head on **goblin** and **dwarf** (opaque-top float gap 6 vs <=3 for human/elf/orc; confirmed by the goblin masculine screenshot), and the non-human crude torsos sit low. The overlay PNGs are drawn full-frame at `BODY_RECT` with no per-body transform, so a helmet baked at a generic head height floats over a lower head. See the [image-production follow-up matrix](#image-production-follow-up-matrix) row for the exact ids/gaps. | Code lane: a data-owned per-body/per-slot overlay transform in `data/player_visuals.json` applied in `player_visual.gd` `_draw_helmet`/`_draw_torso`/`_draw_feet` (default identity, so correct bodies never move), plus an acceptance check for helmet/head contact. Art lane: re-author the mis-baked PNGs (do NOT edit assets in a code lane). | Every body's crude helmet/torso/feet visibly sits on the rig (contact-sheet/screenshot review across all ten bodies); a semi-automated helmet/head-contact check where practical; no gameplay/timing change; no image production in the code lane. |
...
| Mis-anchored crude overlays: `helmet_crude_goblin`, `helmet_crude_dwarf` (helmet baked ~6px too high — opaque top at rows 3/1 while those heads start at rows 9/7); review `torso_crude_<non-human>` and `feet_crude_<non-human>` which also sit low. Only pursue re-authoring where PR-03B decides a code overlay transform cannot align it. | 16x32 transparent | Overlay drawn at a generic head/body height that floats over the shorter goblin/dwarf rigs (PR-03B) | `data/player_visuals.json` rig anchors (`helmet`/`torso`/`feet_y`); `art/generated/player_gear/` | Helmet opaque region overlaps the rig head region for every body (float gap <= ~3px); contact-sheet review across all ten bodies; pixel verifier green; no regression on the already-aligned bodies |
