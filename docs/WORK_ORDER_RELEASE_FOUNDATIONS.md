...
| R-06 | Incremental ownership decomposition | Extract HUD and session services one subsystem at a time. Retire historical HUD fallback paths only after export verification. | Necessary for extension, but not a prerequisite to a safe build. | No behavior/save-format regression; each extracted component has focused coverage; full suite remains green. **SUBSTANTIALLY COMPLETE 2026-07-27** — 3 clean stateless slices shipped (chrome/theme resolver, edit geometry, vessel/chrome texture-prep → `HudChrome` + `HudEditGeometry`); R-06.4/6.5/6.6 assessed and closed as not-beneficial (node-mutation-heavy + directly smoke-driven → child-node lift is net-negative coupling). Authority: `docs/WORK_ORDER_R06_OWNERSHIP_DECOMPOSITION.md`. |
