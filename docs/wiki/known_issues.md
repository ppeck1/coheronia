...
| Light leak through diagonal block seams | With the perception veil on (the default `fog_of_war` world rule; also `COHERONIA_PERCEPTION=1`), underground scenes show **daylight and torch light leaking through 1×1 diagonal block gaps**. This is not the terrain veil — that samples the visibility mask per cell with `filter_nearest` and is crisp — but the underlying 2D lighting: the sun/moon shadow occluders are per-cell squares that touch only at their corners (so a diagonal seam passes a sliver of daylight), and torches/lava are intentionally **shadowless** (they glow *through* rock so a torch carved into a wall does not self-occlude). The brighter/wider torch + lava tuning made the pre-existing leak more visible. | Cosmetic; perception/visibility state and gameplay are unaffected. | Diagonal-corner occluder bridging for the celestial shadows, and/or a selective shadow pass for local emitters; deferred (Perception + Resonance follow-up). |
