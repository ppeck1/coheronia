...
| Swing / action-FX art | Pick/axe/sword swings play a data-driven windup→impact→recovery aimed at the target, but the authored frames read as code-posed rather than hand-authored, and the sword has no bespoke swing family yet. | Mining/combat timing is unchanged; the gap is art fidelity, not motion logic. | Author smoother swing frames / a sword swing family (S-07.1 D4 art lane); the code contract already consumes them. |
