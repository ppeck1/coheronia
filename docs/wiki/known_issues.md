...
|---|---|---|---|
| Pick/axe/sword swing art | PR-04 gave every tool/weapon a data-driven windup->impact->recovery swing aimed at the target (up/down/diagonal). S-07.1b added a generated **sword swing family** (F10, all species/variants via `scripts/art/gen_tool_swing_frames.py`) so the sword renders authored overlay frames like pick/axe, plus a swing-arc strike FX. The pick/axe swing PNGs remain hand-authored three-phase frames. | Mining and combat timing are unchanged; any remaining gap is art fidelity, not motion logic. | Optional: further polish the hand-authored pick/axe frames; the code contract (`action_profile`, `swing_direction`) already consumes authored overlays. |
...
| Calling skill panel | The two Path cards are readable and player-language. S-07.1 removed the spurious horizontal scrollbar (canvas width now clamps to the columns; holds at 640×360). Node/inspector text is still dense. | Cosmetic; the panel is fully usable and gating/purchase are correct. | A UI polish pass over node/inspector density. |
| Character creation at 640×360 | The form (with the Calling selector and its innate/Paths above the fold) fits and scrolls at the small window size, but the base text is uncomfortably tiny. | The screen works; small-window legibility is poor. | Improve the 640×360 layout — larger type / tighter rows for the small viewport (S-07.1b remainder, art/operator lane). |
