# ARCHIVE — Claude Code / Fable Prompt — Coheronia v0.1 One-Shot Build

> **Historical document.** This is the original one-shot prompt that created the
> v0.1 scaffold in July 2026. It is preserved verbatim for provenance only and
> describes a scope the project has long since outgrown (the game now ships
> procedural world depths, liquid physics, a visible citizenry, the Calling
> progression system, adaptive music, and much more). **Do not follow it for
> current work** — see the root [`CLAUDE.md`](../CLAUDE.md), [`README.md`](../README.md),
> and [`docs/HANDOFF.md`](HANDOFF.md) for current state and rules.
