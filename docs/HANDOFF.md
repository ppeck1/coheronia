...
S-07 "no-new-mechanics" boundary below: a deliberate final feature arc before v0.7-alpha.
**As of 2026-08-24 this arc is INTEGRATED into the S-07 stabilization branch
`s07-stabilize-b-plus`** via a real `--no-ff` merge of the arc through `eed93f1` (the
unrelated `d008589` F3-debug commit was deliberately excluded); the arc also still lives
...
**Now integrated with the Perception + Resonance arc above on the same branch
`s07-stabilize-b-plus`.** Five S-07 hardening slices (`14d7da3` smoke leak/focus/verifier
cleanliness, `deb24d5` docs/hygiene, `523d135` v0.7-alpha metadata + RELEASE_CHECKLIST,
`f15b812` JSON-allowlist-loophole close + fail-closed version reader, `96821c2`
`HudInventoryRules` extraction) sit under `05ec3ae`; the perception arc was then merged in.
The S-07 arc is **stabilization and truthfulness**, not new mechanics. Its authority is
