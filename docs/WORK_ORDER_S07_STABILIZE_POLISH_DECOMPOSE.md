...
|---|---|---|---|
| **S-07.0 — Truth & flake classification** | Fix `HANDOFF.md` freshness (F12): state screenshots refreshed in `10cafef`, drop the "confirm before pushed" hedge (all on `main`). Classify the headless `r06_texture_prep_delegates` flake per D1. | `HANDOFF.md` matches git reality; headless smoke is deterministically green **or** the flake is explicitly skipped/xfail with a comment pointing here; windowed 532/532. | (test-meta only) |
| **S-07.1 — Modal presentation & panel polish** | F6 modal occlusion (dim/hide preview/gate input per D2); F7 Calling panel (kill h-scrollbar, cut text density, fix inspector hierarchy); F8 Town Hall (shorten instructions, give roster vertical space, cut competing info); F9 640×360 legibility; F10 improved swing frames (D4). | Modal panels visually isolate; no red placement square behind a modal; Calling panel has no h-scroll; Town Hall roster taller; char-create readable at 640×360; new swing frames land in-game. Windowed smoke green. | `s07_modal_occludes_hud`, `s07_calling_panel_no_hscroll` |
...
| **S-07.3 — Smoke suite decomposition** | Split `smoke_test.gd` (F1) into bounded per-domain files behind a **thin coordinator** that preserves every check name, order-independence, and the `smoke_results.json` schema. Suggested split: `smoke_world_generation.gd`, `smoke_persistence.gd`, `smoke_player_combat.gd`, `smoke_callings.gd`, `smoke_ui_hud.gd`, `smoke_settlement_subjects.gd`, `smoke_assets_and_rendering.gd`. | Coordinator runs all domain files; **check count and names identical** to pre-split (diff the results JSON name-set); no check lost or renamed; windowed green; headers under each file document its domain. | `s07_smoke_coordinator_covers_all` (asserts the union of domain files == the pre-split name-set) |
