...
- **2026-08-28 — Perception restore and stationary-sight correctness.** Restoring an older live save now **replaces** the explored fog bitset instead of unioning it with cells discovered after that save; transient visibility is cleared and recomputed at the restored player position. Perception LOS now also invalidates when the effective integer sight radius changes as darkness eases, so terrain, entity, and light gating update while the player stands still. Dedicated pure-model, live F9-path, and stationary terrain/entity/light regressions pass **608/608** in the local windowed source run and **602/602** in the Windows export (six expected read-only-fixture skips); the full local verifier is green.
