...
`Godot 4.6 · GDScript · data-driven design · 518-check in-engine smoke suite · procedural world depths · adaptive music · layered image-first UI pipeline`
...
**Verify the build** (validators + the 518-check in-engine suite):
...
- **2026-08-04 — The Calling system.** Turned the vestigial "role" concept into a real player-identity progression strand, presented as your **Calling**. Choose one of three permanent, character-owned Callings at creation — **Oathbound** (sworn defender), **Wayfarer** (deep/wild explorer), or **Runewright** (hearth & resonance) — each with an automatic **innate** effect and **two Paths** of twelve tiered skills. Your Calling gates which two Paths you can buy into; tiers open purely by count of skills already bought in that Path (**II at 2, III at 6, capstone at 9**), reusing the existing per-level perk-point economy. The skill panel now renders your Calling, its innate, and its two Paths laid out tier by tier with live owned / available / tier-locked states and an effect inspector. **Effects are honestly scoped:** thirteen skills plus the Oathbound innate are **wired live** — a new damage-**source** tag (enemy / hazard) drives source-scoped hostile & hazard damage reduction (Resolve, Holdfast, Last Watch, Tunnel Hardened, Stormwise), defensive statics (max health, armor amplification, Attunement capacity), faster mining, wider scouting, and on-defeat health/Attunement restores — while every remaining skill is authored with its exact authoritative hook traced in [`docs/CALLING_EFFECT_MATRIX.md`](docs/CALLING_EFFECT_MATRIX.md) (staged) or flagged as needing a new mechanism (deferred), never simulated. **Save-safe:** the serialized key stays `role`, legacy characters auto-map to the default Calling at read time (no save rewrite), and Callings grant no starting-item windfall. Source smoke **518/518** (+3 Calling checks). Authority: [`docs/CALLING_EFFECT_MATRIX.md`](docs/CALLING_EFFECT_MATRIX.md) · [`docs/VARIABLE_MATRIX.md`](docs/VARIABLE_MATRIX.md).
