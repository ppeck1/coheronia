...
- **Two full-smoke assertions are currently red.** The HUD edit-mode direct-manipulation (`fq17_hud_edit_direct_manipulation`) and inventory drag/sort (`fq09_inventory_board_drag_and_sort`) assertions failed in the fresh 2026-07-20 332/334 full run (the live clip-switch check passed that run). The focused inventory smoke passed 5/5, and the board's drag/drop and sort paths are implemented; the harness/runtime regression is tracked as PR-00 in `docs/PRESENTATION_RECOVERY_MATRIX.md` and must be repaired before the suite can be called fully green.
