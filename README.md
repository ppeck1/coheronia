...
`Godot 4.6 · GDScript · data-driven design · 337-check in-engine smoke suite · adaptive music · layered image-first UI pipeline`
...
- **Self-verifying build.** A smoke suite runs the *real game* — real input map, real physics, real saves — and asserts 337 checks across mining, save/load round-trips, legacy migrations, UI panel contents, Map/Events coexistence, HUD-kit layering, physics traversal, armor math, adaptive-music transitions, the character-rendering contract, body-specific gear resolution, and event stingers. The recorded 2026-07-17 inventory-focused run passed 5/5; the full suite is at 337/337 on 2026-07-20 (the real-time `fq09u1_live_clip_switch` adaptive-music check occasionally cold-flakes and passes on rerun).
...
**Verify the build** (validators + the 337-check in-engine suite):
...
- **Gear overlays now resolve robustly (PR-03A); overlay alignment is still being fixed (PR-03B).** The repository ships 120 body-specific PNGs for crude helmet/torso/feet overlays and three-phase basic-pick, forged-pick, and crude-axe swings. These now *resolve* against the character's *effective body id* — the resolved body when one loaded, otherwise the intended body — so authored gear stays visible across character/load/world-transition/forge refresh paths instead of intermittently dropping to the procedural fallback; a `refresh_presentation()` hook re-resolves at the equip/forge boundaries. Overlay *placement* is not fully correct yet: the crude helmet floats above the head on the shorter bodies (goblin, dwarf) and the non-human torsos sit low — tracked as PR-03B. Swing-animation polish (stepped poses, anchors, arc continuity, the missing sword sequence) is also still open. See `docs/CHARACTER_RENDERING_CONTRACT.md` and the known-issues page.
...
- **Full smoke is green (337/337, 2026-07-20).** The HUD edit-mode direct-manipulation (`fq17_hud_edit_direct_manipulation`) and inventory drag/sort (`fq09_inventory_board_drag_and_sort`) assertions were briefly red (332/334) when the inventory-board work landed; both were repaired in `scripts/ui/hud.gd` — the crest widget's default size is now captured from its settled content and the inventory-board cell rebuild removes stale cells before re-adding them. The presentation recovery arc (PR-00 through PR-03) is tracked in `docs/PRESENTATION_RECOVERY_MATRIX.md`.
