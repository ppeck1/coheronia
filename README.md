...
The centered **command tray** — **Goal · Craft · Map · Edit** — opens each panel with a click, so **Crafting** is reachable without knowing the **C** shortcut. The **Crest** (settlement pressures) and **Events** (journal) readouts live in the two wooden **dock wings**; click a wing to open the full detail.
...
| ![The redesigned responsive crafting recipe-book](docs/screenshots/15_crafting.png)<br>*Open crafting with the **Craft** tray button or **C** — icon-led recipe cards across Hand · Town Hall · Workbench · Furnace · Anvil, with a pinned Craft/Build action* | ![The inventory board with loadout, backpack, and dock](docs/screenshots/03_inventory.png)<br>*Open the inventory with **I** — drag and drop stacks, gear, and the hotbar dock* |
...
| ![The player standing on a wooden platform bridging a gap beside the hall](docs/screenshots/38_wooden_platform.png)<br>*The craftable **Wooden Platform** — a one-way plank you jump up through and walk across* | ![Full-height three-tall doors in a settlement wall](docs/screenshots/34_tall_doors.png)<br>*Full-height doors seal a walled room to raise settlement housing* |
...
- **2026-08-28 — Perception correctness and platform alignment.** Restoring an older live save now **replaces** the explored fog bitset instead of unioning it with cells discovered after that save; transient visibility is cleared and recomputed at the restored player position. Perception LOS now also invalidates when the effective integer sight radius changes as darkness eases, so terrain, entity, and light gating update while the player stands still. The Wooden Platform's authored plank and one-way collision surface now occupy the **bottom** of their tile together, removing the false-looking pocket beneath a platform placed directly over solid terrain. Dedicated model, live F9-path, stationary terrain/entity/light, and platform art/collision regressions pass **609/609** in the local windowed source run and **603/603** in the Windows export (six expected read-only-fixture skips); the full local verifier is green.
- **2026-08-28 — Crafting recipe-book redesign, Craft in the command tray, and a media/docs closeout.** The unified crafting panel is rebuilt as a **responsive recipe-book**: icon-led recipe cards showing only **Ready / Missing / Locked** on their face (the full shortage list lives in the detail pane and tooltip), and a detail pane that is a single scrolling column above a **pinned Craft/Build action** — so the button, and the panel's own footer, stay on-screen and usable from a full **1280×720** down to a **640×360** viewport. Station navigation spans **Hand · Town Hall · Workbench · Furnace · Anvil**, with Build cards for unbuilt stations. Crafting is now a first-class, discoverable action: the centered command tray reads **Goal · Craft · Map · Edit**, and the new **Craft** chip opens the same panel as the **C** shortcut through one shared toggle — its pressed state tracks every close route (chip, C, Escape, Close). A craftable **Wooden Platform** joins the ladder — a one-way traversal plank you jump up through and stand on. Plus an authored **−12 dB Music-bus calibration** trim and an inventory **dock zero-count reconciliation**. The published HUD and gameplay gallery was refreshed against the current four-button tray. Cross-platform windowed source smoke is **605/605** and export smoke is **599/599**. The former `perception_resonance_e2e_through_fog` intermittent was a test-fixture isolation defect—not a player-visible fog/Resonance regression—and is resolved; adaptive music now also stops and detaches its composite streams cleanly before shutdown.
